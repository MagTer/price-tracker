...
| 260714-jov | Produktlistans "Senaste pris" → "Lägsta kr/enhet". Kolumnen visade `stores.find(s => s.price_sek != null)` — första länken i en array från en query **utan `ORDER BY`**, alltså icke-deterministisk (inte "senaste"), ojämförbar mellan packstorlekar (24-pack slår alltid 12-pack absolut) och blind för rea (läste `price_sek`, inte erbjudandet). Nu: minimum av serverberäknat `unit_price_sek` (D-03) + vinnande länkens butik/förpackning/effektiva pris, badge för länkar utan mängd, fallback till lägsta absoluta pris märkt "kr/enhet saknas". `store_unit_price_sek` orörd (D-05). Ren frontend — allt fanns redan på tråden | 2026-07-14 | 998a9fd | [260714-jov-lagsta-kr-enhet-i-produktlistan](./quick/260714-jov-lagsta-kr-enhet-i-produktlistan/) |
