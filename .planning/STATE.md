...
- 2026-08-13 D-34: **Tre "öppna spakar" stängda på mätning i stället för på magkänsla.** Alla tre stod bara i CLAUDE.md, aldrig här, och lästes därför som obeslutade. Underlaget är `check_attempts` i prod, 2026-07-27 → 2026-08-13 (395 kontroller: 382 ok, 5 blocked, 8 fetch_failed). (1) **WAF-sidecaren byggs inte.** Alla 5 blockeringar är ICA, fyra av dem samma dag (2026-07-29, före v0.39.0:s stratifiering) och den femte klarnade på fem minuter genom brytarens egen cooldown. **Exakt en av fem låg på människovägen** — quick-add-previewen som utlöste spiken — och ingen sedan dess. En Playwright-sidecar för en händelse var fjortonde dag som självläker är fel pris. Mätningen från 2026-07-29 säger att token FUNGERAR, inte att den BEHÖVS; de fyra fakta den gav ligger kvar i CLAUDE.md om väggarna kommer tillbaka. (2) **curl_cffi är bevisad**, inte längre "promising" — och receptet för att mäta den var fel: `docker logs | grep 'Bot wall'` nollställs vid varje deploy, så en omdeployad dag svarar "0 väggar" och läses som bevis (hände under just den här verifieringen, mot en 20 minuter gammal container). `check_attempts.outcome='blocked'` är den beständiga mätningen. (3) **Döda länkar är hanterade** — `link_health` → Fel & luckor → radering för hand; enda prod-fallet var en ICA-länk som 404:ade fem morgnar och sedan togs bort. Automatisk pensionering är AVGJORT NEJ: varje tröskel raderar en länk under ett butiksavbrott, och URL:en är länkens identitet med historiken hängande i den. (Prod har 100 länkar, 0 inaktiva — inget har någonsin pensionerats i stället för raderats.)
...
- **ICA:s måndagsfönster — flytta det före frukost?** Öppen fråga med mätning bakom sig, och
  ...
  natten 2–3 augusti 2026 (`docs/experiments/ica-night-probe/`) visar att ICA byter priser strax
  ...
  `docs/experiments/ica-night-probe/results.jsonl` — **flyttat in i repot 2026-09-06**, för
  att det låg på en dev-VM som numera går att ersätta med ett kommando, och den här raden
  pekade dit med absolut sökväg. Det städas när fönsterfrågan är avgjord, inte av en
  ombyggnad.
...
| 260817-m4q | Två visningsfel som en modelleringsfråga blottade (hur spårar man en vara som bara finns i EN storlek?). (1) Länkpanelens "Butiken anger" skrev butikens tryckta jfr-pris utan måttenhet — enda cellen i portalen som gjorde det, så ICA:s kr/kg stod som "63,20 kr" bredvid vårt "6,81 kr" i kr/st, två mått i samma rad utan något som skiljer dem. Måttavläsaren flyttades (inte kopierades) från `validation._printed_measure` till `pricing.printed_measure`: de två anroparna bedömer och etiketterar SAMMA tal, och en andra tabell skulle låta en rad renderad "kr/kg" bedömas som kr/st. `unitKr()` är förbjuden i cellen — den etiketterar med PRODUKTENS enhet. (2) Quick-adds mängd överlevde ett byte av kanonisk enhet: 0,27 tolkat ur "…270g" som kg blev kvar som 0,27 STYCK, och `step="1"` fick webbläsaren att vägra med "the two nearest valid values are 0 and 1" — v0.53.3:s form, rätt regel som oläsbart meddelande, mitt på normalvägen för en envariantsprodukt. Båda anropsställena går nu genom `pkgRekeyUnit`; operatörens etikett överlever, en autogenererad följer med mängden. Båda felen osynliga för varje runtime-test — mätta i en node-vm mot det riktiga skriptfragmentet, båda statiska grindarna verifierat röda mot gammal markup. 895 tester (7 nya). **Släppt som v0.58.1** | 2026-08-17 | 5f1f90a, 540be36 | [260817-m4q-butikens-matt-pa-jfr-priset-och-mangdfal](./quick/260817-m4q-butikens-matt-pa-jfr-priset-och-mangdfal/) |
| 260817-r2t | Rapporterat från prod på v0.58.1: en Willys-rad i Fel & luckor påstod 73 % fel med BÅDA talen rätta, och ritade "97,78 kr/st" för en siffra tryckt i kr/kg. Mätt, inte resonerat — prod-DB:n visade `{"source":"willys_api"}` utan mått medan ICA-punkterna hade sitt, och en probe mot Willys API (6 produkter) gav `comparePrice='97,78 kr'` + `comparePriceUnit='kg'` genomgående. Två fel: (1) extractorn läste måttet ur `comparePrice` med en suffix-regex som ALDRIG matchat — varje Willys-punkt som någonsin skrivits saknar mått, så validatorn har dömt alla utan veto; ofarligt så länge `Product.unit` råkade vara det tryckta måttet. Nu läses `comparePriceUnit`, normaliserat till "/kg" (bar token matchar ingen markör). Den gamla kommentaren som påstod sig ha SETT "33,29 kr/kg" rättades på plats — att tro på den var vad som kostade måttet. (2) Fel & luckor etiketterade butikens siffra med produktens enhet — **övervägt och avvisat i v0.58.1** på argumentet att radens premiss gör dem lika; premissen håller bara när ett mått faktiskt registrerats, och prod gav motexemplet inom timmar. Grinden täcker nu BÅDA renderingarna, verifierad röd mot v0.58.1:s markup. 900 tester (5 nya). **Släppt som v0.58.2** | 2026-08-17 | eb074c7, a4283c4 | [260817-r2t-willys-jamforprisenhet-och-felmarkt-matt](./quick/260817-r2t-willys-jamforprisenhet-och-felmarkt-matt/) |
| 260906-d55 | **The ICA night probe moved into the repo, because two lines here pointed at evidence on a disposable machine.** Its conclusion was harvested 2026-08-14 into CLAUDE.md's schedule bullet and this file's "ICA:s måndagsfönster" item — but both referenced the raw data by ABSOLUTE path (`~/ica-natprobe/results.jsonl`) and the repo held no copy. home-server gained `make rebuild-dev` on 2026-09-06, which replaces the dev VM's disk from the image; this file said the data is "städas när frågan är avgjord", and since the window question is still OPEN a rebuild would have settled it by accident and left two dangling pointers in a repo claiming the evidence exists. 28 KB to prevent that. Five files copied byte-identical (`cmp`-verified) into `docs/experiments/ica-night-probe/`, with two edits made because this is versioned source now: `probe.py` resolved `sys.path` from a hardcoded absolute path to one clone and now derives it from its own location (the stated way forward is "re-run the probe wider", which must not require editing it first — resolution verified to land on this repo's `src/`), and the probe README's claim that the data sits outside the repo is corrected in place. Checked before committing: results.jsonl carries only `run/at_utc/label/status/price/offer/offer_details` — no URLs, cookies, fingerprints or IPs — and no `~/ica-natprobe` reference remains in CLAUDE.md, this file or docs/. `docs/spikes/` was rejected as the home: it holds throwaway .py explorations, this is a finished measurement with an open decision on it. **The ICA window is still NOT moved** — n=1 product, the observed move was a campaign ENDING rather than one appearing, and one 03:00 run 404'd. `~/ica-natprobe/` is left in place until the copy is confirmed. ruff + format + 861 passed / 98 skipped — a regression check, not proof, since nothing under src/ or tests/ was touched | 2026-09-06 | (this commit) | .planning/quick/260906-d55-ica-nattproben-in-i-repot |
