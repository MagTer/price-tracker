...
last_activity_desc: "v0.9.0 check-day: quick-add now exposes check_weekday (backend supported it all along, the modal never did) as a Swedish weekday select, prefilled Monday for ICA/Willys via quickadd.OFFER_WEEKDAYS (chain property, no env) — one check/link/week on the offer day, lightest load on the stores. Manual link form's raw 0-6 number input upgraded to the same select. Suite: 328+12. Deploy bump pending (go straight to v0.9.0). Older notes: v0.8.0 shareability (env butik config, MIT, README instance guide, Erbjudanden start page; separate instances over multi-tenant), v0.8.1 footer-version-in-image fix. .env.template still not updatable from here (permission-blocked)."
progress:
  total_phases: 6
  completed_phases: 5
  total_plans: 16
