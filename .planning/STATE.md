...
Last activity: 2026-07-06 — Completed quick task 260706-tha: fixed 4 stale Entra OIDC references in CLAUDE.md to describe the actual IAP header-trust auth model (X-Auth-Request-Email / ALLOWED_ENTRA_EMAIL), attributing Entra enforcement to the upstream Traefik + auth-middleware ingress, now planned to be managed via Dokploy (not yet built, pending Entra client registration).
