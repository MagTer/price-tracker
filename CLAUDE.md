...
**Status (2026-07-26): the extraction is done and this is a live product.** It is deployed in prod. Phase 04.1 — package data moved from `Product` to `ProductStore` — is built, verified, and deployed. Test suite: **538 passing** (that total includes 21 Postgres integration tests; with no DB reachable they skip cleanly and you get `517 passed, 21 skipped`).

**Deploy state (2026-07-26): prod runs `v0.29.1`; `v0.29.3` is tagged and built (v0.29.2 was never deployed on its own), waiting on the deploy bump.** Backup export checked against the live instance (on v0.26.0): 15 products / 38 links / 41 history rows, 38 KB, format 1.1. **This line goes stale the moment a tag is cut, so never trust it on its own** — `git tag | tail -1` is the newest tag and `ssh magnus@192.168.10.223 "sudo docker inspect price-tracker --format '{{.Config.Image}}'"` is what prod actually runs. Deploying is a one-line bump of the pinned tag in the home-server repo's `compose/dokploy-apps/price-tracker/docker-compose.yml` — that repo is not this repo, and the release is not live until that bump lands.
...
- **HTTP client:** httpx (async), with **HTTP/2 enabled** (`httpx[http2]` → `h2`) — see the fetcher bullet: h2 is part of the Chrome fingerprint, not a performance choice
