...
class PriceNotifier:
    ...
    def _deal_row_html(self, deal: DealRow, now: datetime) -> str:
        ...
        # Where the offer applies is a condition on the price like any other, and this
        # mail is read standing in the aisle after a drive. The sentence comes from
        # domain/result.offer_channel_note — never re-worded here.
        if deal.offer_channel_note:
            condition_bits.append(deal.offer_channel_note)
        if condition_bits:
            price_cell += (
