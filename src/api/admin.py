...
async def price_tracker_dashboard(principal: Principal = Depends(get_principal)) -> HTMLResponse:
    ...
    parts = (
        (templates / "admin.html").read_text(encoding="utf-8").split("<!-- SECTION_SEPARATOR -->")
    )
    ...
    # The shell (outer document, sidebar, header, mobile chrome) and the design-system
    # CSS are templates too — they lived as string literals in this module until v0.51.0,
    # which meant the design tokens existed in two languages in two files. Small
    # per-principal strings go in via %%…%% tokens (pre-escaped here); the CSS/HTML/JS
    # blocks via placeholder comments and .replace(), never str.format() — both block
    # types are full of braces.
    shell = (templates / "shell.html").read_text(encoding="utf-8")
    base_css = (templates / "shell.css").read_text(encoding="utf-8")
    ...
    # A reader keeps the role badge: a page whose write buttons are simply absent reads
    # as a broken deploy rather than as a permission level.
    role_badge = "" if principal.is_admin else '<span class="role-badge">Läsbehörighet</span>'

    page = (
        shell.replace("%%BODY_CLASS%%", body_class)
        .replace("%%EMAIL%%", safe_email)
        .replace("%%VERSION%%", f"v{_APP_VERSION}" if _APP_VERSION else "Price Tracker")
        .replace("%%ROLE_BADGE%%", role_badge)
        .replace("%%AVATAR%%", safe_email[0].upper() if safe_email else "?")
